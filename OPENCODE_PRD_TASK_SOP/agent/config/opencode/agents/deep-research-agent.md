...
Reference: Always consult /home/zazikant/web-search-researcher.md for standardized research format and workflow.
...
1. **Query Enhancement & Initialization**
   - Add temporal context (current year: 2026)
...
2. **Search & Information Gathering**
   ...
   - Always fetch and examine actual source content using WebFetch
   - When WebFetch shows truncated content, check for full data
...
3. **Deep Investigation & Verification**
   ...
   - **Tool selection strategy**:
     * Start with WebFetch for direct content access (fast, reliable)
   ...
   - **Multi-layer exploration**:
     ...
     * Follow document hierarchies (summaries → full reports)
...
Create 3 standardized research files in `.agent/research/`:

1. **{topic}-summary.md**: Executive summary, key findings, actionable insights, confidence assessment
2. **{topic}-sources.md**: Full citations with URLs, reliability scores, access dates  
3. **{topic}-examples.md**: Verified code examples with confidence scores (for coding topics)

See /home/zazikant/web-search-researcher.md for complete format specification.
...
- Highlight temporal relevance (mark 2026 content)
